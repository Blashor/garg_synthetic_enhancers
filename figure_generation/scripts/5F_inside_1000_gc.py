...
with open(INSIDE_file) as file:
    ...
    for line in file:
        ...
        gc = 100 * Bio.SeqUtils.gc_fraction(cell[2])
        print(gc)
        line_seq_holder[cell[1]][cell[0]].append((cell[2], gc))
...
with open("../data/INSIDE_genome_sep12_75gens.txt") as file:
    ...
    for line in file:
        ...
        surv_num = (itr % survivors) + 1
        cell = line.split("\t")
        g_num = int(cell[0].split("Gen:")[-1]) + gen_add
        cell[0] = "G" + str(int(cell[0].split("Gen:")[-1]) + gen_add)
        # print(cell[1], cell[0])
        current_stat += float(cell[3])
        cell[1] += "_" + str(surv_num)

        name, rep, surv = cell[1].split("_")
        g_num = g_num - 1
        if g_num < 49:
            if name == "Shh" and rep == "rep1":
                shh[g_num] = 100 * Bio.SeqUtils.gc_fraction(cell[2])
            if name == "Meox1" and rep == "rep1":
                meox1[g_num] = 100 * Bio.SeqUtils.gc_fraction(cell[2])
...
for line in line_seq_holder:
    # print(line)
    sample, gen, rep, surv_num = line.split("_")
    for gen in line_seq_holder[line]:
        ...
        if gen_num > gen_skip and gen_num % gen_skip == 0:
            ...
            last_score = float(line_seq_holder[line][last_gen][0][1])
            score_diff = gen_num
...
for violin in violin_dict:
    ...
    if (np.median(violin_dict[violin]) > 0 and (len(replicate_num_motif_occurs[violin]) / 4) > 5) and violin.split("_")[
        ...
        if int(violin.split("_")[-1]) < 6:
            ...
            clr_med = cmap(norm_score(np.median(violin_dict[violin])))
...
for sample in display_score:
    ...
    if sample == "line256rep6":
        ...
        continue
    all_scores.append(display_score[sample])
    ax1.plot(
...
median = np.median(all_scores, axis=0)
p25 = np.percentile(all_scores, 25, axis=0)
p75 = np.percentile(all_scores, 75, axis=0)

x = range(len(median))

ax1.plot(x, median, color="white", linestyle="-", linewidth=2)
ax1.plot(x, p25, color="white", linestyle=":", linewidth=2)
ax1.plot(x, p75, color="white", linestyle=":", linewidth=2)
...
fig.tight_layout()
plt.savefig("../figures/4c_gc_1000.svg", dpi=200)
plt.show()
