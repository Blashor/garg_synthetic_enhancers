...
prefixes = [
    "FIRE_peaks_stitched",
    "FIRE_peaks_super_med1",
]

"""
prefixes = [
    "osn_control/OSN_stitched_control0",
    "youngStitched_OSN",
    "youngSE_OSN",
]
"""
prefixes = ["controls/FIRE_stitched_control0", "fire_stitched", "med1_Fire"]

prefixes = ["../data/3runsV2"]
...
def main_length():
    ...
    for prefix in prefixes:
        ...
        new_x = numpy.linspace(x[0], x[-1], int(100 * len(x)))
        new_y = numpy.interp(new_x, x, y)

        def powlaw(x, a, b, c):
            return a / numpy.power(x, b) + c

        # 3. Fit the power-law model
        params, covariance = curve_fit(powlaw, new_x, new_y, maxfev=10000)
        a_fit, b_fit, c_fit = params

        # 4. Generate x values for the fitted curve
        x_fit = np.linspace(min(x), max(x), 100)
        y_fit = powlaw(x_fit, a_fit, b_fit, c_fit)

        # 5. Plot the fitted curve
        plt.plot(x_fit, y_fit, color="grey", alpha=0.5, linewidth=2, label=f"Fit: y = {a_fit:.2f} * x^{b_fit:.2f}")
        # plt.legend()
        for x, y, dist in zip(xAxis, yAxis, yAxisDist):
            q25 = np.percentile(dist, 25)
            q75 = np.percentile(dist, 75)
            ax.vlines(x, q25, q75, color="grey", linewidth=0.5)
        # Optionally: print or store the fit parameters
        print(f"Fitted power-law parameters: a = {a_fit:.4f}, b = {b_fit:.4f}, c={c_fit:.4f}")
        # print(len(mat))
