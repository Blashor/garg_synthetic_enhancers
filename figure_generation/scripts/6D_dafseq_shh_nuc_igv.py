...
def string_diff(a, b):
    ...
    for i in range(len(a)):
        if a[i] != b[i]:
            if a[i] in {"A", "T"}:
                if b[i] in {"G", "C"}:
                    change = (0.13, 0.47, 0.71, 1.0)  # AT=>GC
                else:
                    change = (0.98, 0.60, 0.60, 1.0)  # AT=>AT
            elif a[i] in {"G", "C"}:
                if b[i] in {"A", "T"}:
                    change = (0.85, 0.37, 0.01, 1.0)  # GC=>AT
                else:
                    change = (0.55, 0.75, 0.95, 1.0)  # GC=>GC
            ...
            for size, start in zip(sizes.split(","), starts.split(",")):
                ...
                if size > 150:
                    ...
                    for i in range(block_start, block_end):
                        if i < 5000:
                            coverage5000[i - 1] += 1
    coverage5000 = (fiber5000 - coverage5000) / fiber5000
...
for diff in diff20:
    ...
    if d < 218:
        ...
        d = d - 218
        ax[2].axvline(d + 1307, linestyle="--", color=color, alpha=1, linewidth=0.6)
for diff in diff50:
    d, color = diff
    if d < 218:
