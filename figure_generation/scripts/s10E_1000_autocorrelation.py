...
def get_upsample(signal, upsample=2):
    return np.repeat(signal, upsample)


def seq_to_S_binary(b, upsample, filters):
    # diffs = []
    ...
    return np.array(get_upsample(diffs, upsample))


def seq_to_W_binary(b, upsample, filters):
    # diffs = []
    diffs = np.zeros(len(b))
    # filters = {"AA", "TT", "AT", "TA"}
    # filters = {"G", "C"}
...
def pearson_autocorr(seq, max_lag=None, upsample=2, filter_set=None):
    x = np.array(seq_to_S_binary(seq, upsample, filter_set))  # assume 1 for S, 0 otherwise
    y = np.array(seq_to_W_binary(seq, upsample, filter_set))
    N = len(x)
    if max_lag is None:
        max_lag = N - 1
    result = []
...
def pearson_autocorr(seq, max_lag=None, upsample=2):
    S = np.array(seq_to_S_binary(seq, upsample))  # assume 1 for S, 0 otherwise
    W = np.array(seq_to_W_binary(seq, upsample))  # assume 1 for W, 0 otherwise
    ...
    for lag in range(max_lag + 1):
        if lag == 0:
            # overlap at same position (usually meaningless biologically)
            probs.append(1)
        else:
            S_i = S[:-lag]
            W_j = W[lag:]
            ...
            if np.sum(mask) == 0:
                ...
                stat = np.sum(W_j[mask] == 1) / np.sum(mask)
                probs.append(stat)
...
def multi_sequence_periodicity(seqs, max_lag=45, label=0, filter_set=None):
    upsample = 1
    total_corr = np.zeros(upsample * max_lag)
    ...
    for seq in seqs:
        ...
        corr = pearson_autocorr(seq, upsample * max_lag, upsample, filter_set=filter_set)[: upsample * max_lag]
    ...
    print(n)
    plt.plot(
        np.array(range(0, len(avg_corr))) / upsample,
        avg_corr,
        linestyle,
        label=f"{alpha+1}",
        color="black",
    ...
    for w in range(17):
        for i in range(0, 60, 10):
            ...
            for ei, e in enumerate(enh_strings):
                ...
                seqs.append(scramble_string(seq_2))
