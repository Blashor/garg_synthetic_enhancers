...
def net_x(graphing_obj, node_scores, motif_sizes, generations, motif_appears_in_replicates):
    ...
    # sys.exit()
    norm_weights = [(weight) / (40) * (max_alpha - min_alpha) + min_alpha for weight in weights]
    ...
    norm_gens = np.array(gen_values) - min([0])
    print(len(pos))
    node_pos = {
        "SOX2": 1 - 0.5,
        "POU5F1": 2,
    ...
    spacing = 0  # controls how far apart duplicates are

    for node in pos:
        node_select = node.split(" ")[0]

        base = node_pos.get(node_select, 0) * 5

        # increment count for this TF
        counts[node_select] += 1
        idx = counts[node_select] - 1

        # center duplicates around base (instead of drifting upward)
        offset = (idx - (counts[node_select] - 1) / 2) * spacing
    ...
    min_dist = 15  # minimum allowed distance between nodes
    iterations = 100
        ...
        for i in range(len(nodes)):
            for j in range(i + 1, len(nodes)):
                ...
                if dist < min_dist and dist > 0:
                    ...
                    pos[n1][1] -= dy_norm * push
                    pos[n2][1] += dy_norm * push
...
with open("../data/INSIDE_40_definitive/fimo.tsv") as file:
    for line in file:
        ...
        if cell[0] != "motif_id" and len(cell) > 5:
            ...
            # print(cell)
            if float(cell[-3]) < 10**-6 and gen_n < 150:
                ...
                if motif.replace("PO", "POU") in allowed_motifs:
