...
plt.figure()
np_heat = np_heat[2:, :512]
for low_res_bool in (True, False):
    if low_res_bool == True:
        low_res = np_heat.reshape(298, 16, 32).mean(axis=2)
        x_tick_positions = np.arange(2, 17, 2)
    else:
        low_res = np_heat
        x_tick_positions = np.arange(128, 512, 128)
        plt.xlabel("Position (bp)")
    sns.heatmap(low_res, cmap="Greys")
