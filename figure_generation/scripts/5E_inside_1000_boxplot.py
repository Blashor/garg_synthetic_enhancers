    ...
    for line in file:
        ...
        if g_num < 49:
            if name == "Shh" and rep == "rep1":
                shh[g_num] = math.sqrt(float(cell[3]))
            if name == "Meox1" and rep == "rep1":
                meox1[g_num] = math.sqrt(float(cell[3]))
    ...
    for gen in line_seq_holder[line]:
        ...
        if gen_num > gen_skip and gen_num % gen_skip == 0:
            ...
            for motif in motif_holder:
                ...
                violin_dict[motif_id].append((gen_num))
...
# convert to array (samples × positions)
