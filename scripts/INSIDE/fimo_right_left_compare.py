...
for motif in motif_set:
    for gen in pop_level_data:
        if motif in pop_level_data[gen]:
            if motif == "POF51":
                print(
                    gen,
                    pop_level_data[gen][motif]["both"],
                    pop_level_data[gen][motif]["right"],
                    pop_level_data[gen][motif]["left"],
                )
                jaccard = pop_level_data[gen][motif]["both"] / (
                    pop_level_data[gen][motif]["both"]
                    + pop_level_data[gen][motif]["right"]
                    + pop_level_data[gen][motif]["left"]
                )
                print(motif, jaccard, sep="\t")
                motif_set[motif].append(jaccard)
        else:
            motif_set[motif].append(0)

for motif in motif_set:
    if max(motif_set[motif]) > 0.1:
        print(motif, motif_set[motif])
        sns.lineplot(x=range(len(motif_set[motif])), y=motif_set[motif], label=motif)
plt.legend()
plt.show()
