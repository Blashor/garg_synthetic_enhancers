    ...
    for gen in line_seq_holder[line]:
        ...
        if gen_int == 150:
            ...
            for i1, motif1 in enumerate(motifs):
                if motif1[0] == "SOX2" or motif1[0] == "PO5F1" or motif1[0] == "NANOG":
                    motifs_nums += 1
                    # motif1[0] = "OSN_scramble"
                    # motif1[1] = random.randint(1, 1011)
            # print(gen, motifs)
...
for line in line_seq_holder:
    ...
    for gen in line_seq_holder[line]:
        ...
        gen_mod = gen_int % gen_skip
        # print(gen)
        if gen_int % 30 == 1 and gen_int != 1:
            gen30range += 30
        if gen_int <= 150:
            print(gen_int, gen30range)
            if gen_int != 1:
                gen_int = gen30range
...
for motif_id in ecdf_holder:
    ...
    for gen_int in ecdf_holder[motif_id]:
        ...
        if np.sum(gc) > 0:
            ...
            data_array = smooth_array(gc / total, 2)  # - smooth_array(gc0 / total0, 4)  # shape (1024,)
            ...
            # Step 3: Compute distances between peaks
            peak_distances = [peak_indices[i + 1] - peak_indices[i] for i in range(len(peak_indices) - 1)]
            sns.ecdfplot(data=peak_distances)
            plt.show()
            # Plot histogram
            x_values = list(range(-256, -256 + len(data_array)))
            """
            plt.plot(
                x_values, data_array, color="black", alpha=(gen_int / 260) + 0.1, linewidth=1, label=f"Gen: {gen_int}"
            )
            """
            plt.ylim([0.0, 1.0])
