...
def output_parents_to_file(scored_parents, line_ids):
    for itr, l_id in enumerate(line_ids):
        parent = scored_parents[0][itr]
        print(l_id, parent[0], str(parent[1]), sep="\t")


def parse_parents(p_file):
    line_ids = []
    parent_seqs = []
...
def main(setup_obj, parent_file):
    ...
    line_ids, parent_seqs = parse_parents(parent_file)
    # seqs_to_bedANDfasta(parent_seqs, fasta_info)
    # run_prediction()
    scored_parents = score_predictions(parent_seqs, fasta_info)
    output_parents_to_file(scored_parents, line_ids)
...
working_dir = "../INSIDE/" + sys.argv[1] + "_no_selection/"
