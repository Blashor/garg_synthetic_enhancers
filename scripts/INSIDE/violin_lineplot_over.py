...
fimo_folder = "/Users/blake/Documents/gargLab/INSIDE/INSIDE_1000"  # fimo_40_160 "INSIDE_40_definitive/fimo.tsv"
...
for line in line_seq_holder:
    print(line)
    sample, surv_num, rep, other = line.split("_")
...
norm_score = mcolors.Normalize(vmin=0, vmax=60)
...
for sample in display_score:
    ...
    ax1.plot(range(len(display_score[sample])), display_score[sample], label=sample, alpha=0.8)
...
sm.set_array([])
cbar = fig.colorbar(sm, orientation="horizontal", pad=0.1, ax=ax2, shrink=0.5)
