...
# INSIDERSS.py
#
# in silico directed evolution of sequence via residual sum of squares
#
...
def score_predictions(gen_obj, fasta_info, scored_parents=None):
    ...
    for line_i in range(len(gen_obj)):
        ...
        for seq_i in range(len(gen_obj[line_i])):
            ...
            stat = 0
            for track, track_weight in enumerate(track_weight_arr):
                if track_weight != 0:
                    residuals = preds[stat_itr, :, track] - preds_baseline[stat_itr, :, track]
                    square_residuals = numpy.square(residuals)
                    rss = numpy.sum(square_residuals)
                    stat += track_weight * rss
