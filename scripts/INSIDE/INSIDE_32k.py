...
def seqs_to_bedANDfasta(next_generation_obj, fasta_info):
    ...
    for line in next_generation_obj:
        ...
        for seq in line:
            ...
            bed_lines.append(seq_id + "\t0\t32768\t\n")
...
def region_setup(enh_len, margin, center_len):
    ...
    half_loci_size = int((32768 - (seq_len + center_len)) / 2)
...
def setup_obj_helper():
    setup_obj = {}
    example_line = {
        "survivors": 4,
        "children_per": 500,
        "mut_rates": [2.11],  # new setup of number of mutation, percent that are dups
        "enh_len": 250,
        "margin": 50,
        "center_len": 900,
    }
    ...
    mut_rates = []  # [1.01, 1.004][0.004, 1.004, 2.004]"""
